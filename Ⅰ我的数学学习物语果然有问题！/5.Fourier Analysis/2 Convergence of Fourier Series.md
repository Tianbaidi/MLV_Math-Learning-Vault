...
此外，这些内积被称为 **埃尔米特内积** ，我们会发现他**不符合**此前定义出来的**对称性** 
...
![[Pasted image 20260402223646 1.png]]
...
$$||X+Y||\leq ||X||+||Y||$$
**proof .**  我们对 1. 的证明只需要构造 $(X+Y,X+Y)$ 后按照范数和正交性就你能得证；
   对于 2. 这称为柯西-施瓦兹不等式，我们如果令其中一个变量的范数为 $0$ ,那么我们很容易猜到 $|(X,Y)|=0$ ,我们令 $t$ 为任意的实数，有 
$$0\leq ||X+tY||^{2}=||X||^{2}+2t\mathrm{Re}(X,Y)$$
这里 $\mathrm{Re}$ 指的是实数，如果我们的 $(X,Y)\neq 0$ 如果 $t$ 取的足够大（或者足够小）这将于我们原来的假设矛盾。同样的，如果我们的取 $||X+itY||^{2}$ 我们也能得到 $\mathrm{Im}(X,Y)=0$ .
我们已经验证了 $||Y||=0$ 的情况，若 $||Y||\neq 0$ 我们能得到什么结果呢 ?  **构造正交化**
我们巧妙设 $c= \frac{(X,Y)}{(Y,Y)}$ . 于是我们得到 $X-cY$ 与 $Y$ 正交（自然与 $cY$ ）正交

> [!NOTE] 神奇吧
> 当我们设 $c$ 等于这个神奇的玩意后，相当于将 $(X-cY,Y)$ 变成了 $(X,Y)-(cY,Y)=0$ 于是正交 

   我们这里利用 The Pythagorean Theorem ，并且将 $X$ 设为 $X-cY+cY$ 得到 
$$||X||^{2}=||X-cY||^{2}+||cY||^{2}\geq |c|^{2}||Y||^{2}$$
   两边开方得到 , 得到我们要的结果，并且当且仅当 $X=cY$ 时取到等号

对于 3. 我们注意到 
$$||X+Y||^{2}=(X,X)+(X,Y)+(Y,X)+(Y,Y)$$
我们可以利用上一个结论，$|(X,Y)+(Y,X)|\leq 2||X||\cdot||Y||$ 于是有 
$$||X+Y||^{2}\leq ||X||^{2}+2||X||\cdot ||Y||+||Y||^{2}=(||X||+||Y||)^{2}$$
$Q.E.D$

## 伟大的希尔伯特空间

> 我们讨论了一些有限维的情况，这种空间被我们称为 **欧几里得空间** 。我们现在将维度拓为无限，你这个应该也能想到，那么只能说你发现了非常伟大的东西！（但是希尔伯特空间可以不是无限的，这个引非常不严谨，但是就这了解一下他们之间的差别）

![[Pasted image 20260406152247.png]]

好的，我们从定义出发来聊聊 **希尔伯特空间** 和 **预希尔伯特空间** 

**Definition.2** 如果一个内积空间是 **完备** 的，我们就称之为 **希尔伯特空间** (Hilbert Space)  
- 完备 ( complete )：我们要验证以下几点：
  1. 
- 内积是严格正定的，即 $\| X\| = 0$ 蕴含 $X = 0$。